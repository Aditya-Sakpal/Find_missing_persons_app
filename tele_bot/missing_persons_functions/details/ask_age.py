...
from tele_bot.missing_persons_functions.details.ask_address import ask_address
import boto3
...
def ask_age(message,img_data_param,name_param):
    if (message.content_type == 'text'):
        if re.match(r"^\+?\d{10,15}$",message.text):

            contact=message.text
            logger.info("Asking for person's age from ask_age.py line 17")
            ...
            try:
                ...
                logger.exception("Exception occured at ask_age.py line 22")
                bot.send_message(message.chat.id,"आंतरिक सर्वर त्रुटि")   
                bot.register_next_step_handler(message,partial(handle_exp))       
        else:           
            logger.info("Asking user to enter valid person's number from ask_age.py line 25")    
            try:
                bot.send_message(message.chat.id, "अवैध फोन नंबर। कृपया पुन: प्रयास करें।")
                bot.register_next_step_handler(message,partial(ask_age,img_data_param=img_data_param,name_param=name_param))
            except Exception:
                logger.exception("Exception occured in ask_age.py line 30")
                bot.send_message(message.chat.id,"आंतरिक सर्वर त्रुटि")   
                bot.register_next_step_handler(message,partial(handle_exp))   
        ...
        if face_detected == "Invalid image":
            logger.info("Asking user to enter the image with face from ask_age.py line 48")
            ...
            def ask_phone_ag(message,img_data_param):
                if(message.content_type == 'text'):    
                    name=message.text
                    ...
                    try:
                        ...
                        logger.exception("Exception occured in ask_age.py line 52")
                        bot.send_message(message.chat.id,"आंतरिक सर्वर त्रुटि")   
                        bot.register_next_step_handler(message,partial(handle_exp))     
                    ...
                    if face_detected == "Invalid image":
                        logger.info("Asking user to enter the image with face from ask_age.py line 75")
                        bot.send_message(message.chat.id, f"कृपया एक चेहरा वाली छवि दर्ज करें")   

                    elif not face_detected:
                        bot.send_message(message.chat.id, f" कृपया व्यक्ति का नाम दर्ज करें.")
                        bot.register_next_step_handler(message, partial(ask_phone_ag,img_data_param=img_data))         
                    else:
